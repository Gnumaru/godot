...
def include_file_in_gles2_header(filename: str, header_data: GLES2HeaderStruct, depth: int):
        ...
        while line:
            ...
            while line.find("#include ") != -1:
                ...
                if included_file not in header_data.vertex_included_files and header_data.reading == "vertex":
                    ...
                    if include_file_in_gles2_header(included_file, header_data, depth + 1) is None:
...
def build_gles2_header(filename: str, shader: str) -> None:
    include_file_in_gles2_header(shader, header_data := GLES2HeaderStruct(), 0)
    out_file_class = (
        os.path.basename(shader).replace(".glsl", "").title().replace("_", "").replace(".", "") + "ShaderGLES2"
    )
...
class {out_file_class} : public ShaderGLES2 {{
public:
""")
...
def build_gles2_headers(target, source, env):
    env.NoCache(target)
    for src in source:
        build_gles2_header(f"{src}.gen.h", str(src))
